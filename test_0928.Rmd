...
```{r}
p + geom_point(aes(population/10^5, total), size = 3) +
  geom_text(aes(population/10^5, total, label = abb), nudge_x = 1.5)
```
...
#> d. Because we want all colors to be blue, we do not need to map colors, just use the color argument in `geom_label`.
p <- ggplot(data = murders)
p + geom_point(aes(population, total), color = "blue") +
  geom_text(aes(population, total, label = abb))
```
...
p <- ggplot(data = murders)
p + geom_point(aes(population, total), color = "blue") +
  geom_text(aes(population, total, label = abb), color = "blue")
```
...
```{r}
p <- murders %>% ggplot() +
  geom_point(aes(population, total, col = region)) +
  geom_text(aes(population, total, label = abb, col = region))
print(p)
```
